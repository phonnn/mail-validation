...
@Injectable()
export class AAAAResolverService extends DnsResolverAbstract<string, AAAAResolutionResult> {
  constructor(cacheProvider: AbstractCacheProvider) {
    super(cacheProvider, 'aaaa');
  }
  ...
  async resolve(domain: string): Promise<DnsResolverResult<string>> {
    ...
    if (cached) {
      this.logger.debug(`AAAA cache hit for ${domain}`);
        return {
          records: cached,
        ...
        try {
          this.logger.debug(`AAAA cache miss for ${domain}, resolving from DNS`);
      ...
      this.logger.error(`AAAA resolution failed for ${domain}:`, error);
      return {
        records: [],
        ttl: this.getRecordTypeTtl(),
        resolutionTime: Date.now() - startTime,
  ...
  async performDnsResolution(domain: string): Promise<AAAAResolutionResult> {
    try {
      this.logger.debug(`Performing AAAA DNS resolution for ${domain}`);
      const records = await dnsResolveAaaa(domain, { ttl: true });
      ...
      this.logger.debug(`Found ${ipAddresses.length} AAAA records for ${domain} with TTL: ${minTtl}s`);
  ...
  protected getRecordTypeTtl(): number {
    // AAAA records typically have shorter TTL (1 hour)
    return 60 * 60; // 1 hour
  }
}
